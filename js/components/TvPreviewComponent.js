export default {
    ...
    <div>
        <div class="preview-section-lightbox">
            ...
            <video class="preview-section-lightbox-video" controls>
                <source :src="'video/' + singleContent[0].tv_video" type="video/mp4">
            </video>
        </div>
        <section class="preview-section">
                ...
                <div class="container preview-section-container">
                    <div class="preview-content">
                        <h1>{{singleContent[0].tv_name}}</h1>
                        <div class="preview-content-line">
                            <h3>{{singleContent[0].tv_genre}}</h3>
                            <h3>{{singleContent[0].tv_date}}</h3>
                        </div>
                        <p>{{singleContent[0].tv_desc}}</p>
                    </div> 
                </div>
        </section>
    </div>
    ...
    methods: {
        getContentData(singleid) {
            const url = `./includes/index.php?getSingleTv=${singleid}`;
            fetch(url)
            .then(res => res.json())
            .then(data => {
        ...
        checkLike(){
            const url = `./includes/index.php?checkLike=1&userId=${this.$root.$data.user['user_id']}&contentType=tv&contentId=${this.singleId}`;
            fetch(url)
            .then(res => res.json())
            .then(data => {
                if(data === false){
        ...
        likeItem(){
            if(this.itemLiked){
                // remove like from db
                const url = `./includes/index.php?removeLike=1&userId=${this.$root.$data.user['user_id']}&contentType=tv&contentId=${this.singleId}`;
                fetch(url)
                .then(res => res.json())
                .then(data => {
                    this.itemLiked = false;
                ...
                // add like to db
                const url = `./includes/index.php?addLike=1&userId=${this.$root.$data.user['user_id']}&contentType=tv&contentId=${this.singleId}`;
                fetch(url)
                .then(res => res.json())
                .then(data => {
                    this.itemLiked = true;
